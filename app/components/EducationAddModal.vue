...
interface Emits {
  ...
  (e: 'created', education: Education): void
}
...
async function handleSubmit() {
  ...
  try {
    const education = await $fetch<Education>(`/api/portfolios/${props.portfolioId}/education`, {
      method: 'POST',
      body: {
    ...
    toast.add({
      title: 'Успешно',
      description: 'Образование добавлено',
      color: 'success'
    })
    ...
    const message = fetchError.data?.message || 'Не удалось добавить образование'
    toast.add({
      title: 'Ошибка',
      description: message,
      color: 'error'
